...
def Generate_Plot(field,region_mesh,array_type,base_electrode_mesh,df_electrode_mesh):
    ...
    SubPlot_Number = 1
    plotter = pv.Plotter(shape=(int(SubPlot_Number), int(SubPlot_Number)))
    plotter.set_background('black')  # Changing background to black

    for i in range(SubPlot_Number):
        r, c = divmod(i, int(SubPlot_Number))  # Calculate row and column index
        plotter.subplot(r, c)
...
mesh = pv.read('/home/cogitatorprime/sandbox/TI_Pipeline/Custom_Python_FEM/tTIS/Simple_Export_Save_Dir/fem_model-name_12-13-10-11.vtk').slice(normal=[0, 0, 1], origin=[0, 0, 0])
...
if 'mat_id' in mesh.cell_arrays:
    ...
    fields = ['potential_base', 'potential_df']
    colormaps = ['coolwarm']
