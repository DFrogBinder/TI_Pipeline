...
MONTAGE_PRESET="${TI_MONTAGE_PRESET:-right-m1}"
...
sbatch \
    --array="$ARRAY_SPEC" \
    --export=ALL,TI_REPAIR_PLAN_FILE="$REPAIR_PLAN_FILE",TI_SIM_RUNNER_PY="$SIM_RUNNER_PY",TI_COMPLETION_CHECK_PY="$COMPLETION_CHECK_PY",TI_MESH_TIMEOUT_HOURS="$MESH_TIMEOUT_HOURS",TI_MESH_MAX_RETRIES="$MESH_MAX_RETRIES",TI_MONTAGE_PRESET="$MONTAGE_PRESET" \
    "$@" \
    "$BATCH_SCRIPT"
