...
TI_INDIVIDUALIZED_TARGETS_CSV="${TI_INDIVIDUALIZED_TARGETS_CSV:-}"
TI_EXPECTED_INDIVIDUALIZED_TARGETS_SHA256="${TI_EXPECTED_INDIVIDUALIZED_TARGETS_SHA256:-}"
if [ -n "${TI_INDIVIDUALIZED_TARGETS_CSV}" ] && \
   [ -z "${TI_EXPECTED_INDIVIDUALIZED_TARGETS_SHA256}" ]
then
    echo "[ERROR] Individualized targets CSV requires its expected SHA-256." >&2
    exit 2
fi
...
if [ -n "${TI_INDIVIDUALIZED_TARGETS_CSV}" ] && \
   [ ! -f "${TI_INDIVIDUALIZED_TARGETS_CSV}" ]
then
    echo "[ERROR] Individualized targets CSV is missing: ${TI_INDIVIDUALIZED_TARGETS_CSV}" >&2
    exit 2
fi
...
if [ "${STAGE}" = "mesh" ]; then
    ...
    ARRAY_EXPORTS="ALL,${COMMON_EXPORTS},TI_COHORT_STAGE=simulate,TI_COHORT_MANIFEST=${TI_COHORT_SIMULATION_MANIFEST},TASK_OFFSET=${OFFSET},TI_TARGETS_CSV=${TI_TARGETS_CSV},TI_EXPECTED_TARGETS_SHA256=${TI_EXPECTED_TARGETS_SHA256},TI_INDIVIDUALIZED_TARGETS_CSV=${TI_INDIVIDUALIZED_TARGETS_CSV},TI_EXPECTED_INDIVIDUALIZED_TARGETS_SHA256=${TI_EXPECTED_INDIVIDUALIZED_TARGETS_SHA256},TI_SIM_RUNNER_PY=${TI_SIM_RUNNER_PY},TI_COMPLETION_CHECK_PY=${TI_COMPLETION_CHECK_PY}"
fi
