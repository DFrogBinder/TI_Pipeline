...
INDIVIDUALIZED_TARGETS_CSV=$(python3 -c 'import json,sys; value=json.load(open(sys.argv[1])).get("individualized_targets_csv"); print(value or "")' "${SUMMARY}")
INDIVIDUALIZED_TARGETS_SHA256=$(python3 -c 'import json,sys; value=json.load(open(sys.argv[1])).get("individualized_targets_csv_sha256"); print(value or "")' "${SUMMARY}")
MONTAGE_MODE=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get("montage_mode","fixed_roi"))' "${SUMMARY}")
STUDY_ID=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get("study_id","unknown"))' "${SUMMARY}")
if [ -n "${INDIVIDUALIZED_TARGETS_CSV}" ]; then
    if [ ! -f "${INDIVIDUALIZED_TARGETS_CSV}" ] || \
       [ -z "${INDIVIDUALIZED_TARGETS_SHA256}" ]
    then
        echo "[ERROR] Individualized target table or its expected hash is missing." >&2
        exit 2
    fi
    ACTUAL_INDIVIDUALIZED_TARGETS_SHA256=$(sha256sum "${INDIVIDUALIZED_TARGETS_CSV}" | awk '{print $1}')
    if [ "${ACTUAL_INDIVIDUALIZED_TARGETS_SHA256}" != "${INDIVIDUALIZED_TARGETS_SHA256}" ]; then
        echo "[ERROR] Individualized target table hash mismatch." >&2
        exit 2
    fi
elif [ "${MONTAGE_MODE}" != "fixed_roi" ]; then
    echo "[ERROR] Preflight declared ${MONTAGE_MODE} without an individualized target table." >&2
    exit 2
fi
...
if [ -n "${INDIVIDUALIZED_TARGETS_CSV}" ]; then
    echo "[INFO] Individualized TI:   ${INDIVIDUALIZED_TARGETS_CSV}"
    echo "[INFO] Individualized SHA:  ${INDIVIDUALIZED_TARGETS_SHA256}"
fi
