...
| `<roi>_fixed_neighbor_efield_overlay.png` | Visual QC overlay: TI/e-field values cropped to the fixed-template neighbor regions, with sparse faint colored neighbor markers and unchanged region contours retained for anatomical attribution. Written only when a T1 background is available. |
