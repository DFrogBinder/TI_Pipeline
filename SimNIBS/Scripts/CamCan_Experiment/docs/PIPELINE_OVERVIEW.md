...
- **Atlas override**: `python post/run_post_processing.py --atlas-filename /path/to/shared_atlas.nii.gz` reuses one atlas for every subject, while `python post/run_post_processing.py --atlas-filename mri/aparc.a2009s+aseg.nii.gz` looks for each atlas at `<fastsurfer_root>/<subject>/mri/aparc.a2009s+aseg.nii.gz`.
- **ROI selection**: `post/run_post_processing.py` can infer the FastSurfer target ROI from the dataset root name via `utils/roi_registry.py`.
  ...
  Cortical aliases such as `right_m1` and `right_dlpc` now resolve to direct Destrieux/a2009s labels for direct ROI-to-ROI comparison; explicit `_dkt` aliases remain available for legacy DKT/coarse atlas fallbacks.
  If no ROI alias is recognized, the batch runner exits before starting analysis for that dataset.
- **Launch examples**:
  - Local: `python post/run_post_processing.py`
  - HPC (single node): `sbatch HPC_scripts/run_post_processing.slurm`
