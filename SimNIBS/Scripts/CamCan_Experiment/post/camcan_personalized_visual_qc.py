...
def _audit_legacy_overlays(
    metrics: Mapping[str, Any],
    overlay_paths: Sequence[Path],
) -> dict[str, Any]:
    """Audit legacy plots without overriding the main pipeline's QC contract.

    ``run_post_process`` deliberately treats overlay-only plotting failures as
    non-blocking.  Metrics and the common-scale paired QC remain valid even
    when Nilearn cannot render one of the optional single-field panels.
    """
    ...
    if missing_files:
        raise RuntimeError(
            "Post-processing returned legacy overlay path(s) that do not exist: "
            + ", ".join(missing_files)
        )

    subject_meta = metrics.get("subject_metrics_meta", {})
    qc_meta = metrics.get("qc_meta", {})
    checks = qc_meta.get("checks", {}) if isinstance(qc_meta, Mapping) else {}
    overlay_qc = checks.get("overlays", {}) if isinstance(checks, Mapping) else {}
    if not isinstance(overlay_qc, Mapping):
        overlay_qc = {}

    expected_count = int(overlay_qc.get("expected_overlay_count", 7))
    written_count = len(overlay_paths)
    reported_written_count = int(
        overlay_qc.get("written_overlay_count", written_count)
    )
    if reported_written_count != written_count:
        raise RuntimeError(
            "Legacy overlay QC metadata disagrees with returned output paths: "
            f"metadata={reported_written_count}, returned={written_count}."
        )

    blocking_checks = (
        list(subject_meta.get("blocking_qc_checks", []))
        if isinstance(subject_meta, Mapping)
        else []
    )
    nonblocking_checks = (
        list(subject_meta.get("nonblocking_qc_checks", []))
        if isinstance(subject_meta, Mapping)
        else []
    )
    if "overlays" in blocking_checks:
        raise RuntimeError(
            "The main post-processing pipeline classified legacy overlays as "
            "a blocking QC failure."
        )

    return {
        "status": str(overlay_qc.get("status", "unknown")),
        "expected_count": expected_count,
        "written_count": written_count,
        "missing_overlay_types": list(
            overlay_qc.get("missing_overlay_types", [])
        ),
        "nonblocking": "overlays" in nonblocking_checks,
    }
...
def _run_full_post_task(task: FullPostTask) -> dict[str, Any]:
    ...
    legacy_overlay_audit = _audit_legacy_overlays(metrics, overlay_paths)
    ...
    payload = {
        ...
        "legacy_overlay_audit": legacy_overlay_audit,
        "legacy_overlay_policy": (
            "Legacy single-field overlay-only plotting failures are "
            "non-blocking when the main post-processing pipeline reports "
            "subject_metrics_meta.status=complete. Every returned overlay "
            "path must exist. Common-scale paired PNGs and reports remain "
...
def run_pair_qc(
    *,
    ...
    summary = {
        ...
        "legacy_overlay_expected_max": 20 * 7,
        "legacy_overlay_missing": (20 * 7)
        - sum(len(record["legacy_overlay_paths"]) for record in records),
...
Use the common-scale paired figures for that purpose.  The main post-processing
pipeline treats an overlay-only Nilearn rendering failure as non-blocking; the
index records all legacy panels that were actually produced.  The common-scale
paired PNGs and PDF reports remain mandatory for every in-scope field.
"""
...
def collect_qc(
    *,
    ...
    manifest = {
        ...
        "legacy_overlay_expected_max": EXPECTED_POST_RECORDS * 7,
        "legacy_overlay_missing": (EXPECTED_POST_RECORDS * 7)
        - sum(int(summary["legacy_overlays"]) for summary in summaries),
        "legacy_overlay_policy": (
            "Optional single-field overlay-only rendering failures follow the "
            "main post-processing pipeline's non-blocking QC contract. Every "
            "returned legacy path is validated; all common-scale paired PNGs "
            "and reports are mandatory."
        ),
        "excluded_out_of_scope_personalized_simulations": 200,
