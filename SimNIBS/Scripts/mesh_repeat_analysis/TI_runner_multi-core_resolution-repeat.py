...
# Optional: deterministic mesh refinement via meshmesh sizing-field.
# If MESH_TARGET_SIZE_MM is set (CLI or env), we will run `meshmesh` after
# CHARM has produced the label image (tissue_labeling_upsampled.nii.gz) and
# overwrite the head mesh with a higher-resolution one.
MESH_TARGET_SIZE_MM: float | None = None
MESH_OUTSIDE_SIZE_MM: float | None = None
MESH_SCOPE: str = "brain"  # "brain" or "all"
...
def _write_uniform_sizing_field(
    *,
    label_img_path: str,
    out_path: str,
    target_size_mm: float,
    scope: str = "brain",
    brain_labels: set[int] | None = None,
) -> str:
    """Create a deterministic sizing-field NIfTI for meshmesh.

    meshmesh can accept a "sizing field" image (NIfTI) via --sizing_field. The
    voxel values encode the desired edge length (in mm) at that location.
    Voxels with value 0 fall back to the default settings file behavior.

    We keep it deterministic by:
      * using nearest-neighbor label sampling (we do NOT resample),
      * writing float32 values,
      * selecting voxels purely by label values.

    Parameters
    ----------
    label_img_path:
        Path to tissue_labeling_upsampled.nii.gz (or equivalent).
    out_path:
        Where to write the sizing field NIfTI.
    target_size_mm:
        Target element size (mm) for selected voxels.
    scope:
        "brain" => only WM/GM/CSF (by default labels {1,2,3}) get target size.
        "all"   => all non-zero labels get target size.
    brain_labels:
        Optional override for which labels count as "brain".
    """
    if target_size_mm <= 0:
        raise ValueError("target_size_mm must be > 0")
    if scope not in {"brain", "all"}:
        raise ValueError("scope must be 'brain' or 'all'")

    img = nib.load(label_img_path)
    labels = np.asarray(img.dataobj).astype(np.int16, copy=False)

    if brain_labels is None:
        brain_labels = {1, 2, 3}  # WM, GM, CSF (SimNIBS conventional)

    if scope == "all":
        sel = labels > 0
    else:
        sel = np.isin(labels, list(brain_labels))

    sizes = np.zeros(labels.shape, dtype=np.float32)
    sizes[sel] = float(target_size_mm)

    out = nib.Nifti1Image(sizes, img.affine, img.header)
...
def process_subject(subject_entry, *, repeat_tag: str | None = None):
    ...
    if meshPresent:
        ...
        # Optional: increase mesh resolution deterministically by re-meshing
        # tissue_labeling_upsampled.nii.gz with meshmesh + a sizing field.
        if MESH_TARGET_SIZE_MM is not None:
            try:
                sizing_field_path = os.path.join(
                    subject_dir,
                    f"m2m_sub-{subject.split('-')[-1].upper()}",
                    "label_prep",
                    f"elem_sizes_{MESH_SCOPE}_{MESH_TARGET_SIZE_MM:g}mm.nii.gz",
                )
                _write_uniform_sizing_field(
                    label_img_path=charm_seg_map_path,
                    out_path=sizing_field_path,
                    target_size_mm=float(MESH_TARGET_SIZE_MM),
                    scope=MESH_SCOPE,
                )

                # Overwrite the mesh file that the rest of this pipeline uses.
                # NOTE: meshmesh CLI supports: meshmesh <label_img> <out.msh>
                # and (in SimNIBS 4.x) a --sizing_field NIfTI.
                meshmesh_cmd = [
                    "meshmesh",
                    charm_seg_map_path,
                    fnamehead,
                ...
                run_cmd(meshmesh_cmd, cwd=str(subject_dir), label="meshmesh_refine")
                log_event(
                    "mesh_refined",
                    subject=subject,
                    target_size_mm=float(MESH_TARGET_SIZE_MM),
                    scope=MESH_SCOPE,
                    sizing_field=sizing_field_path,
                    mesh_out=fnamehead,
                )
            except Exception as e:
                log_event("error", stage="meshmesh_refine", subject=subject, error=str(e))
...
def main():
    ...
    # --- Mesh resolution controls (meshing stage) ---
    parser.add_argument(
        "--mesh-target-size-mm",
        type=float,
        default=None,
        help=(
            "Optional: re-mesh tissue_labeling_upsampled.nii.gz with meshmesh using a "
            "uniform sizing-field of this target element size (mm). Smaller => higher resolution. "
            "If omitted, uses CHARM defaults."
        ),
    )
    parser.add_argument(
        "--mesh-scope",
        ...
        default=None,
        help=(
            "Which voxels get the target size when --mesh-target-size-mm is set: "
            "'brain' applies to labels {1,2,3} (WM/GM/CSF), 'all' applies to all non-zero labels. "
            "Defaults to env SIMNIBS_MESH_SCOPE or 'brain'."
        ),
    )

    args = parser.parse_args()

    # Pull meshing controls from CLI or env. We keep them as globals to avoid
    # threading them through every helper in this script.
    global MESH_TARGET_SIZE_MM, MESH_SCOPE
    env_mesh_target = os.environ.get("SIMNIBS_MESH_TARGET_SIZE_MM")
    if args.mesh_target_size_mm is not None:
        MESH_TARGET_SIZE_MM = float(args.mesh_target_size_mm)
    elif env_mesh_target:
        MESH_TARGET_SIZE_MM = float(env_mesh_target)

    env_mesh_scope = os.environ.get("SIMNIBS_MESH_SCOPE")
    if args.mesh_scope is not None:
        MESH_SCOPE = args.mesh_scope
    elif env_mesh_scope:
        MESH_SCOPE = env_mesh_scope
    start = time.time()
