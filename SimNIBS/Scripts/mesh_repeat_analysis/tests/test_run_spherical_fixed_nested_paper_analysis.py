...
def test_full_analysis_writes_six_figures_and_download_bundle(tmp_path: Path) -> None:
    ...
    assert any(name.endswith("nested_per_mesh_repeatability.csv") for name in names)
    assert any(name.endswith("nested_variance_components.csv") for name in names)
    assert not any("residual_matrix" in name for name in names)
